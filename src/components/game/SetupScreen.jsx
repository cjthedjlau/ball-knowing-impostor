    ...
    >
      ...
      <div className={`${darkMode ? 'bg-[#0a0f1e]' : 'bg-white'} px-5 safe-top pb-6 sticky top-0 z-10 shadow-sm`}>
        <div className="flex items-center justify-between max-w-lg mx-auto">
          <div className="flex flex-col items-center text-center flex-1">
            ...
            <p className={`text-xs mt-1 font-medium ${darkMode ? 'text-white/35' : 'text-slate-400'}`}>Sports party game · Find the imposter</p>
          </div>
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 space-y-4">
        ...
        <div className={`${card} rounded-2xl p-5`}>
          ...
          {/* Active team packs indicator */}
          {selectedTeamPacks.length > 0 && (
            <motion.div
              initial={{ opacity: 0, y: -4 }}
              animate={{ opacity: 1, y: 0 }}
              className={`mt-2 flex flex-col gap-1 px-3 py-2 rounded-xl ${darkMode ? 'bg-[#3b82f6]/10' : 'bg-blue-50'}`}
            >
              {selectedTeamPacks.map(tp => (
                <div key={tp} className="flex items-center gap-2">
                  <span className="text-sm">📦</span>
                  <span className={`text-xs font-bold ${darkMode ? 'text-[#3b82f6]' : 'text-blue-600'}`}>{tp}</span>
                </div>
              ))}
            </motion.div>
          )}
